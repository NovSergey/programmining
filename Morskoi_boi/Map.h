...
private:
	...
	bool check_distance(Ship new_ship, vector<Ship> ships) {
		for (auto s : ships) {
			for (auto p : s.position) {
				auto key = p.first;
				if (new_ship.position.count({ static_cast<char>(key.first + 1), key.second }) > 0)
					return false;
				else if (new_ship.position.count({ static_cast<char>(key.first - 1), key.second }) > 0)
					return false;
				else if (new_ship.position.count({ static_cast<char>(key.first), key.second + 1 }) > 0)
					return false;
				else if (new_ship.position.count({ static_cast<char>(key.first), key.second - 1 }) > 0)
					return false;
			}
...
public:
	...
	bool add_ship(Ship ship, vector<Ship> ships) {
		if (ship.position.size() == 0) {
			return false;
		}
		if (check_correct(ship) and check_distance(ship, ships))
		{
			if(is_hide)
